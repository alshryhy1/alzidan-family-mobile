...
export function TreeScreen({
  ...
  const searchResults = useMemo(
    () => searchAllBranches(branches, parents, childrenRows, searchQuery),
    [branches, childrenRows, parents, searchQuery],
  );
  ...
  const openSearchResult = (result: TreeSearchResult) => {
          ...
          {searchQuery.trim().length >= 2 ? (
            <View style={styles.searchResults}>
              {searchResults.length ? (
                ...
                <Text style={styles.searchEmpty}>لا يوجد اسم مطابق في جميع الفروع.</Text>
              )}
            </View>
          ) : null}
        </View>
